...
Long sessions get their older exchanges moved out of the active log on compact, so opening a session stays fast no matter how long it has run. What the user sees:

- A **"Load earlier messages"** row appears above the conversation only for sessions that actually have archived history; it loads one archive chunk per click, oldest-newer order preserved, and becomes **"No earlier messages"** once everything is loaded
- Scrolling to the very top of the message area triggers the same load automatically (re-armed only after scrolling back down, so one gesture doesn't cascade through the whole archive), and the reading position is preserved when the older messages prepend — the view doesn't jump
- Loaded history sits in a **collapsed** block ("Archived · N segment(s) · M messages") — the user scrolled up for older context, not to have hundreds of exchanges re-flow the view; expanding renders them like normal messages
- Archived exchanges are **read-only**: Copy still works, Delete is absent (the server refuses it for archived sessions)
- Chunks already loaded aren't re-fetched; a failed load can simply be retried with the same row
