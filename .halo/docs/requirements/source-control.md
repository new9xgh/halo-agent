...
- **Reconnect reconciliation**: `file:changed` is a fire-and-forget push, so anything that happened while the socket was down (laptop lid, network drop) is simply lost. Panel, history graph and Explorer decorations therefore also refresh once on WS *re*connect (first connect is skipped — the initial load already fetched), which is the only way a terminal commit made during the outage shows up without a manual refresh.
