...
How to build the macOS desktop app (`Halo-0.1.4-arm64.dmg`). The desktop
...
Output: `packages/desktop/dist/Halo-0.1.4-arm64.dmg` (~143 MB).
...
- **A `--x64` / `--win` pack also emits a stray `Halo-0.1.4-arm64.dmg` — it's
  ...
  The real per-target artifacts are `Halo-0.1.4.dmg` (x64) and `Halo Setup
  0.1.4.exe` (win); only trust the arm64 dmg from an actual `dist:arm64` run.
