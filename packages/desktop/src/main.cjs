...
function createWindow() {
  // Window/taskbar icon — in dev the exe is stock electron.exe, so the icon
  // must be set per-window (packaged builds get it from the bundle). The
  // file is generated by gen-icon.mjs; skip gracefully when absent.
...
ipcMain.handle('halo:notify', (e, payload) => {
  ...
  if (Notification.isSupported()) {
    const notification = new Notification({ title: title || '元轴', body: body || '' })
...
// under the app menu; other platforms get NO application menu — the default
// File/Edit/View/Window/Help bar is just chrome noise around a single-page
// workspace (copy/paste accelerators keep working without it).
...
app.on('ready', async () => {
  // Admin defaults to the light theme; tell the OS so the window chrome
  // (title bar, traffic-light area) follows suit.
